						...
						<table id="__01" width="729" height="475" border="0" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<div style="line-height:30px; margin-top:20px ">
												...
												if total = 0 then 
												Dim Tarray:Tarray = UICon.QueryData("select count(*) as total from user_zpxx where "&strwhere&"")
												...
												'Dim sql:sql = UICon.QueryPageByNum("categoryID,id,title,posttime","user_zpxxs", ""&strwhere&"", "ID", true,current_page,pagesize)
												'这种方式为根据IndexID排序，IndexID值越大越靠前
												Dim sql:sql = UICon.QueryPageByNotIn("*","user_zpxx", ""&strwhere&"", "ID", "indexID desc,ID",false,current_page,pagesize)
												'response.Write sql
												'response.End()
												...
												<span style="font-family:'宋体';">·</span><a href="zpxx_in.asp?categoryid=<%=rsn("categoryID")%>&amp;itemid=<%=rsn("id")%>" class="y_0"><%=rsn("title")%></a> </td>
												</tr>
												</table>
												<%
												rsn.movenext
