							...
							<tr>
								...
								<div id="neiye_main">
								 ...
								 CategoryID = VerificationUrlParam("CategoryID","int","0")%>
												...
												if total = 0 then 
													Dim Tarray:Tarray = UICon.QueryData("select count(*) as total from user_cfyl where "&strwhere&"")
												...
												Dim sql:sql = UICon.QueryPageByNotIn("*","user_cfyl", ""&strwhere&"", "ID", "IndexID desc,ID",false,current_page,pagesize)
												'response.Write sql
												'response.End()
								...
								<a href="cfyl_in.asp?categoryID=<%=rs("categoryID")%>&amp;itemid=<%=rs("id")%>" ><img src="<%=rs("Field_picture")%>" width="150" height="110"/></a>
																		<%else%>
																		<a href="cfyl_in.asp?categoryID=<%=rs("categoryID")%>&amp;itemid=<%=rs("id")%>" class="pro_pic">暂无缩略图</a>
																		<%end if%>
																		</td>
																	</tr>
																	<tr>
																	  <td height="23"   align="center" valign="middle"><span style=""><a href="cfyl_in.asp?categoryID=<%=rs("categoryID")%>&amp;itemid=<%=rs("id")%>"><%=rs("Title")%></a></span></td>
																	</tr>
																	</table>	
													</td>
													<% 
