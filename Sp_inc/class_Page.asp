...
%>
	<%=Total_Text%><%=total_record%><%=Record_Unit%>
	<%=current_page%>/<%=total_page%><%=Page_Unit%>
<%
end if
%>
<%if current_page>1 then:Response.Write "<a href='"& GetUrl(total_record) &"1'>"&First_Text&"</a>":else:Response.Write ""&First_Text&"":end if%>
<%if current_page<=1 then:response.Write ""&Pre_Text&"":else:Response.Write "<a href='"& GetUrl(total_record) & (current_page-1) &"'>"&Pre_Text&"</a>":end if%>
<%
if showpageNum then
	...
	for PageIndex=pagestart to PageEnd
		Response.Write "<a href="& GetUrl(total_record) & PageIndex &">"
...
%>
<%if Cint(current_page) < Cint(total_page) then:Response.Write "<a href='"& GetUrl(total_record) & (current_page+1) &"'>"&Next_Text&"</a>":else:Response.Write ""&Next_Text&"":end if%>
<%if Cint(current_page) < Cint(total_page) then:Response.Write "<a href='"& GetUrl(total_record) & total_page &"'>"&Last_Text&"</a>":else:Response.Write ""&Last_Text&"":end if%>
</p>
<%
End sub 
'分页函数结束
